...
export default function ReportUploadPage({ params }: ReportUploadPageProps) {
  ...
  const [sampleValues, setSampleValues] = useState<Map<number, SampleValueInput>>(new Map());
  ...
  const handleSampleValueChange = (testDetailsId: number, value: string) => {

    
    setSampleValues(prev => {
      const newMap = new Map(prev);
      newMap.set(testDetailsId, { testDetailsId, value });
      return newMap;
    });
  };
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody>
                          {samples.map((sample, idx) => (
                            <tr key={`sample-${sample.testDetails.id}`} className={idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                              <td className="py-2 px-2 text-gray-900 font-medium">{sample.childTestName || sample.parentTestName}</td>
                              <td className="py-2 px-2">
                                <input
                                  type="text"
                                  value={sampleValues.get(sample.testDetails.id)?.value || ''}
                                  onChange={(e) => handleSampleValueChange(sample.testDetails.id, e.target.value)}
                              ...
                              <td className="py-2 px-2">
                                <button
                                  onClick={() => handleSaveSample(sample.testDetails.id, sampleValues.get(sample.testDetails.id)?.value || '', sample.investigationId, sample.unit || '', sample.referenceRange || '')}
