...
interface BillingData {
  ...
  labInfo: labInfo;
  billingDetails?: { tot_amt?: number; net_amt?: number; balance_amt?: number; adv_amt?: number; balance_pymnt2?: number; discount?: number } | null;
}
...
export default function BillingPage({ params }: BillingPageProps) {
    ...
    const fetchBillingData = async () => {
      try {
        ...
        const response = await fetch(
          `/api/lab/billing?patient_id=${patient_id}&medical_num=${medical_num}`,
        );
  ...
  // Calculations: use test details sum, or fallback to billing tot_amt when tests list is empty
  const totalAmount =
    (patientData?.patientTestDetails?.length ?? 0) > 0
      ? patientData.patientTestDetails.reduce(
          (sum, test) => sum + test.price,
          0,
        ) || 0
      : patientData?.billingDetails?.tot_amt
        ? Number(patientData.billingDetails.tot_amt)
        : 0;
  ...
  const balanceAmount =
    parseFloat(balAmountState.toString()) > 0
      ? 0
      : Math.max(0, netAmount - advancePaymentNum);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const payload = {
        ...
        discount_percentage: discountEnabled
      ...
      setSaveMessage({
        type: "success",
        text: "Billing changes saved successfully!",
      });
      setLoading(true);
  ...
  const { patientQueue, patientDepDetails, patientTestDetails, labInfo } =
    patientData;
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div ref={billContentRef} className="bg-white print:bg-white">
            ...
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  {labInfo.laboratory_address}, {labInfo.lab_city},{" "}
                  {labInfo.lab_state}
                </p>
              </div>
              <div className="text-right text-sm text-gray-700 space-y-1">
                <p>
                  ...
                  {labInfo.laboratory_phone}
                </p>
                <p>
                  <span className="font-semibold">Email:</span>{" "}
                  {labInfo.laboratory_email}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="border border-slate-300 print:border print:border-gray-400 rounded overflow-hidden">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-100 border-b border-slate-300 print:border-b print:border-gray-400">
                  <tr>
                    <th className="p-3 text-xs font-bold text-slate-700 uppercase w-12">
                      S.No
                    </th>
                    <th className="p-3 text-xs font-bold text-slate-700 uppercase">
                      Tests
                    </th>
                    <th className="p-3 text-xs font-bold text-slate-700 uppercase text-center w-32">
                      Schedule
                    </th>
                    <th className="p-3 text-xs font-bold text-slate-700 uppercase w-40">
                      Instructions
                    </th>
                    <th className="p-3 text-xs font-bold text-slate-700 uppercase text-right w-24">
                      Price
                    </th>
                    <th className="p-3 text-xs font-bold text-slate-700 uppercase text-center w-28">
                      Status
                    </th>
                  </tr>
                </thead>
          ...
          <div className="px-6 py-6 border-t-2 border-slate-300 print:border-t print:border-gray-400">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-1 text-slate-600 text-xs">
                <p className="font-bold uppercase mb-2 text-slate-700">
                  Notice:
                </p>
                <p>
                  This is a computer-generated billing report. Please ensure all
                  tests match the physician's referral.
                </p>
              </div>
                      ...
                      <div className="border border-slate-300 print:border print:border-gray-400 p-4 rounded space-y-2">
                        <label className="block">
                          ...
                          <p className="text-xs text-slate-500 mt-1">
                            Enter amount up to ${netAmount.toFixed(2)}
                          </p>
                        </label>
                      </div>
                    )}
          ...
          <div className="px-6 py-4 border-t-2 border-slate-300 print:border-t print:border-gray-400 text-center text-xs text-slate-500 font-medium">
            Generated by {patientQueue.firstName || "System"} • Original Entry:{" "}
            {format(new Date(patientQueue.createdOn), "dd MMM yyyy")}
          </div>
        </div>
      </div>
