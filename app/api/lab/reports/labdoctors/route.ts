...
export async function GET(req: NextRequest) {
  ...
  if (!testId) {
    return NextResponse.json(
      { success: false, message: 'Missing testId parameter' },
      { status: 400 }
    );
  }
  ...
  try {
    // First, fetch laboratory_id from referral_patient_test_details using testId
    const [labRows]: any = await conn.query(
      'select laboratory_id from referral_patient_test_details where ID = ?',
      [testId]
    );

    if (!labRows.length) {
      return NextResponse.json(
        { success: false, message: 'Test record not found' },
        { status: 404 }
      );
    }

    const laboratory_id = labRows[0].laboratory_id;

    // Now fetch doctors for the laboratory
    const query = `select laboratory_doctors_id as id, CONCAT(doc_firstname, ' ', doc_lastname) as name, 
    doc_dept as department, doc_phone_number as phoneNo, doc_email as email from laboratory_doctors where laboratory_id = ?`;
    
    const [rows]: any = await conn.query(query, [laboratory_id]);
